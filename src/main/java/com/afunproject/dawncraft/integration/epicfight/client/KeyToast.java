...
public class KeyToast implements Toast {
    ...
    public Visibility render(PoseStack poseStack, ToastComponent component, long time) {
        ...
        MutableComponent key = Component.literal(mapping.getTranslatedKeyMessage().getString().toUpperCase(Locale.US));
        mc.font.draw(poseStack, Component.translatable(message + ".title", key), 32.0F, 7.0F, 0x9E0CD2);
        mc.font.draw(poseStack, Component.translatable(message + ".text0", key), 32.0F, 18.0F, -1);
        mc.font.draw(poseStack, Component.translatable(message + ".text1", key), 32.0F, 29.0F, -1);
